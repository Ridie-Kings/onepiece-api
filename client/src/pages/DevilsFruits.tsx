...
import { DevilFruitType } from "../types/interfaces";
...
export const DevilsFruitsPage = () => {
  ...
  const fetchDevilFruits = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error("Error al obtener las frutas del diablo");
      const data = await response.json();
      const filteredData = data.filter(
        (fruit: DevilFruitType) =>
          fruit.name !== "Unknown" && fruit.name !== "None"
      );
  ...
  return (
    <Container>
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl px-4 py-12">
        {fruits.map((fruit: DevilFruitType) => (
          <SecondCard
            item={fruit}
            key={fruit.id}
            link={`/devils-fruits/${fruit.id}`}
            images={[

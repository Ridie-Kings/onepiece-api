export const Carrusel = ({ crew }) => {
  return (
    <section className="flex w-full h-[600px] overflow-x-auto gap-2 py-8 px-4 scrollbar-hide">
      {crew.members.map((member) => (
        ...
        >
          <img
            ...
            alt={member.name}
            className="h-full w-28 object-cover object-top opacity-80 transition-all duration-500 ease-out group-hover:opacity-100 group-hover:w-80 group-hover:contrast-125"
          />
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            ...
            <p className="text-sm font-semibold uppercase tracking-wider text-red-400">{member.role}</p>
